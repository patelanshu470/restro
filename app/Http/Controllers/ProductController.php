...
class ProductController extends Controller
{
    public function index($id)
    {
        ...
        #Recommendation product according to product restaurent top 20 only
        $recommend_product = Product::where([['restaurent_id', '=', $product->restaurent_id], ['id', '<>', $id]])->limit(20)->latest()->get();

        #Getting this product Reviews
        ...
        $products = DB::table('products')
            ...
            ->where([['orders.payment_status', '=', 'captured'], ['products.id', '<>', $id]])  //change "confirm" string according to Payment Method
        ...
        foreach ($get_product as $product_datas) {
            if (in_array($product_datas->id, $uniqueIds)) {
                ...
                $trending_product[] = $product_datas;
            }
        }
        #For sorting according to Count value
        usort($trending_product, function ($a, $b) {
        ...
        return view('pages.user.product.view', compact('trending_product', 'product_review', 'product', 'recommend_product'));
    }

    public function listing(Request $request)
    {

        // work when user search from nav bar
        if ($request->search_text) {
            #search based on name
            $get_product = Product::where('name', 'like', '%' . $request->search_text . '%')->get();

            #search based on restaurent name
            if (count($get_product) == 0) {
                $get_product = null;
                $get_product = Product::join('restaurants', 'products.restaurent_id', '=', 'restaurants.id')
                    ->where('restaurants.restaurant_name', 'like', '%' . $request->search_text . '%')->get('products.*');
            }
            #search based on category name
            if (count($get_product) == 0) {
                $get_product = null;
                $get_product = Product::join('categories', 'products.product_category', '=', 'categories.id')
                    ->where('categories.name', 'like', '%' . $request->search_text . '%')->get('products.*');
            }
            #search based on subcategory name
            if (count($get_product) == 0) {
                $get_product = null;
                $get_product = Product::join('sub_categories', 'products.subcategory_id', '=', 'sub_categories.id')
                    ->where('sub_categories.name', 'like', '%' . $request->search_text . '%')->get('products.*');
            }
            if (count($get_product) == 0) {
                $get_product = null;
                $get_product = Product::where('type', 'like', $request->search_text)->get();
            }
        } else {
            ...
            if ($request->restaurant != '') {
                $result = $result->where('restaurent_id', $request->restaurant);
            }
            ...
            $get_product = $result->where('status',1)->get();

            if (!$request->has('name') && !$request->has('restaurant') && !$request->has('category') && !$request->has('type')) {
                $get_product = null;
                $get_product = Product::where([['status','=',1]])->get();
            }

            if ($request->sort_option == "low-to-high") {
                // $get_product = null;
                $get_product = Product::orderBy(DB::raw('CAST(final_price AS DECIMAL(10,2))'), 'asc')->where([['status','=',1]])->get();
            } elseif ($request->sort_option == "high-to-low") {
                // $get_product = null;
                $get_product = Product::orderBy(DB::raw('CAST(final_price AS DECIMAL(10,2))'), 'desc')->where([['status','=',1]])->get();
            }
        }
        ...
        return view('pages.user.product.list', compact('product', 'cart_product', 'restaurant', 'category'));
    }
                        ...
                        <div class="card-body">
                            <div class="d-flex profile-img1">
                                <div class="profile-img31">
                                    <img src="' . asset('images/product/thumbnail/' . $products->path) . '"
                                ...
                                <div>
                                    ...
                                    <h6 class="mb-1 mt-3 heading-title fw-bolder">' . $products->name . '</h6>
                                    <p class="mt-2 mb-0 pb-4 iq-calories">' . $products->restaurants->restaurant_name . '</p>
                                ...
                                <svg width="18" viewBox="0 0 30 30" fill="none"
                                    ...
                                    <div class="d-flex mt-2">
                                        <span class="text-primary me-4">$' . $products->final_price . '</span>
                                        <a href="#" role="button" data-bs-toggle="modal" data-bs-target="#exampleModalCenter' . $products->id . '" class="opencart' . $products->id . '">
    ...
    public function price_search(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($request->price == "low-to-high") {
                ...
                $product = $get_product->sortBy('final_price');
                // dd($product);
                foreach ($product as $pro) {
                    $img = Attachment::where('attachable_id', $pro->id)->get();
                    foreach ($img as $key => $images) {
            ...
            if ($request->price == "high-to-low") {
                $output = "";
                $get_product = Product::all();
                $product = $get_product->sortByDesc('final_price');
                foreach ($product as $pro) {
                    $img = Attachment::where('attachable_id', $pro->id)->get();
                    foreach ($img as $key => $images) {
                        if ($images->field_name == "product_thumbnail") {
