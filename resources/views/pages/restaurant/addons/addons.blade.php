          ...
          <div class="col-sm-12">
             ...
             <div class="modal fade" id="exampleModalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                   <div class="modal-content" style="background-image: url({{ asset('images/modal_images/addon.jpg') }}); background-position: center; background-repeat: no-repeat; background-size: cover;">
                      ...
                      <div class="modal-body">
                        <form method="POST" action="{{ route('addons.store') }}" id="validation">
                           ...
                           <div class="row">
                             ...
                             <div class="form-group col-md-12">
                                <label class="form-label" for="price" style="color:white">Price:</label>
                                <input type="number" class="form-control input-color" name="price" id="price" placeholder="Price" style="background: transparent;color: #fff;">
                             </div>
                             <div class="form-group col-md-12">
                                {{-- attention:  --}}
                                <label class="form-label" for="price" style="color:white">Cost Price:</label>
                                <input type="number" class="form-control input-color" name="cost_price" id="cost_price" placeholder="cost price" style="background: transparent;color: #fff;">
                             </div>
                           </div>
                        </div>
                      ...
                      <div class="modal-body">
                        <form method="POST" action="{{ route('addons.update',$addons->id) }}" id="edit_addons{{ $addons->id }}">
                           ...
                           <div class="row">
                              <div class="form-group col-md-12">
                                 <label class="form-label" for="fname" style="color:white">Name:</label>
                                 <input type="text" class="form-control input-color" name="name" value="{{ $addons->name }}" id="name" placeholder="Name" style="background: transparent;color: #fff;">
                              </div>

                             <div class="form-group col-md-12">
                                <label class="form-label" for="price" style="color:white">Price:</label>
                                <input type="number" class="form-control input-color" value="{{ $addons->price }}" name="price" id="price{{$addons->id}}" placeholder="Price" style="background: transparent;color: #fff;">
                             </div>
                             <div class="form-group col-md-12">
                                <label class="form-label" for="price" style="color:white">Cost Price:</label>
                                <input type="number" class="form-control input-color" value="{{ $addons->cost_price }}" name="cost_price" id="cost_price{{$addons->id}}" placeholder="cost price" style="background: transparent;color: #fff;">
                             </div>
                           </div>
                        </div>
                        <div class="modal-footer" style="justify-content: center !important; border-top:0 !important">
...
@foreach ($addon as $addons)
{{-- for stoping user to entering -ve no edit form --}}
<script>
    $(document).ready(function() {
      $('#price{{$addons->id}}').on('input', function() {
        var value = parseFloat($(this).val());
        if (isNaN(value) || value < 0) {
          $(this).val('');
        }
